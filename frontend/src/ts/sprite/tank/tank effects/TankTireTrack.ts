...
export class TankTireTrack {
    ...
    public vanishFullTrack(){
        while (!this._listOfTirePairs.isEmpty()) {
            const head = this._listOfTirePairs.head;
            if (head !== null) {
                this._vanishingListOfTirePairs.addToHead(head);
            }
    ...
    public backwardUpdate(topPoint: Point, bottomPoint: Point, hullAngle: number, sin: number, cos: number){
        ...
        if (this._listOfTirePairs.length > TankTireTrack.AMOUNT_OF_CHAINS) {
            const tail = this._listOfTirePairs.tail;
            if (tail !== null) {
                this._vanishingListOfTirePairs.addToHead(tail);
            }
