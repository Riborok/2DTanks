...
export abstract class HullEntity extends RectangularEntity {
    protected constructor(x0: number, y0: number, width: number, height: number, angle: number, mass: number) {
        super(x0, y0, width + TRACK_INDENT, height + (TRACK_INDENT << 1), angle, mass);
    }
...
export class HullModel0 extends HullEntity {
    private static readonly OWN_MASS: number = 1;
    protected _armor: number = 100;
    protected _health: number = 100;
    protected _armorStrength: number = 1;
    public constructor(x0: number, y0: number, angle: number, mass: number) {
        super(x0, y0, HULL_WIDTH[0], HULL_HEIGHT[0], angle, mass + HullModel0.OWN_MASS);
    }
}